...
# fauxG isn't used anymore even though it's input to different functions.  
# Was used for previous analysis with priors.
fauxG = getFauxG()
...
for(i in 1:length(inflates)) {
  ...
  for(j in 1:length(inflates)) {
    ...
    fit = fitModel2(initParams=initPar, dStar=dStar, gpsDat=threshSlipDat, 
                    ...
                    latRange=c(minLat, maxLat), corGPS=TRUE)
    endPar = fit$MLEs
