...
fitCorrelo = function() {
  ...
  residHMax = aperm(allHMax, c(2, 3, 1))
  datPerRep = dim(residHMax)[1] * dim(residHMax)[2]
  residHMax = t(matrix(residHMax, nrow=datPerRep))
  Mw10 = 10^Mw
  MwFin = rep(Mw10, datPerRep)
  topoFin = t(matrix(rep(topo, 5), ncol=5))
  model = lm(residHMax ~ Mw10 + topoFin)
  model = lm(residHMax ~ MwFin)
  for(i in 1:dim(allHMax)[1]) {
    floodVals = allHMax[i,,]
    Mw10Fin = rep(10^Mw[i], datPerRep)
    model = lm(floodVals ~ topo)
